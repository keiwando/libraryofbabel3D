...
public class Book : Escapable {
	...
	void Start () {
		librarian = GameObject.Find("Librarian").GetComponent<Librarian>();
	...
	public override void EscapeClicked() {
		...
		gameObject.GetComponentInParent<Shelf>().deactivate();
		gameObject.GetComponentInParent<Shelf>().GetComponentInParent<Wall>().deactivate();
		print ("back");
		activated = false;
